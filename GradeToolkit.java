...
public class GradeToolkit {
  ...
  */
  static double calculateGPA(Scanner input) {
      ...
      if (count == 0) {
    	  System.out.println("No grades entered, can't calculate GPA");
      } else {
    	  gpa = sum / count;
    	  System.out.printf("You entered %d grades. GPA = %f\n", count, gpa); 
      }
  ...
  */
  ...
  static void calculate_weighted_grade(Scanner input) {
      ...
      if (weights.size() > 0){
          for (int x = 0; x < weights.size(); x++) {
			  ...
			  if (grade != -1) {
			                float weightedGrade = grade * weights.get(x);
  ...
  static void calculate_qpa(Scanner input){
    ...
    float qpa = (float) totalGrades / totalCredits;
